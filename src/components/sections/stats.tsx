...
/** Requirements §S-5. CountUp (M-10) arrives in Phase 4. */
    ...
    <Section id="stats" labelledBy="stats-heading">
      ...
      <dl className="mt-10 grid gap-px overflow-hidden rounded-card border border-border bg-border sm:grid-cols-2 lg:grid-cols-4">
        {stats.items.map((item) => (
          <div key={item.label} className="bg-bg-elev p-6">
            <dt className="sr-only">{item.label}</dt>
            <dd>
              <span className="block font-mono text-[clamp(2.5rem,5vw,3.5rem)] leading-none font-medium tracking-[-0.04em] text-accent">
                {item.value}
                {item.suffix ? (
